...
WageAssumption = ['0.5MP', "Rigid Real", "Rigid Nominal", 'Production Linkages Only', 'Labor Market Frictions Only']
####Setting up shock ####
sec_to_shock = 'dur'
shock_size = 0.01
...
num = 0
dlog_p = nominal_rigidity_network.PriceFunc(dlog_A=dlog_A, dlog_H=dlog_H, dlog_K=dlog_K, Psi=Psi, 
                                            epsN=epsN, epsK=epsK, curlyQ=curlyQ, curlyT=curlyT, 
                                            curlyF=curlyF, curlyL=curlyL, num=num)
dlog_theta = nominal_rigidity_network.ThetaFunc(dlog_A=dlog_A, dlog_H=dlog_H, dlog_K=dlog_K, 
                                                dlog_p=dlog_p, Psi=Psi, epsN=epsN, epsK=epsK, curlyL=curlyL, 
                                                curlyQ=curlyQ, curlyT=curlyT, curlyF=curlyF, num=num)
occT_vec[:-1, i] = dlog_theta.flatten()
dlog_y = multi_occupation_network.OutputFunc(dlog_A, dlog_H, dlog_K, dlog_theta, dlog_lam, Psi, Omega, curlyQ, curlyF, epsN, epsK, curlyT, curlyE)
sectorY_vec[:-1, i] = dlog_y.flatten()
sectorY_vec[-1, i] = multi_occupation_network.AggOutputFunc(dlog_y, dlog_lam, dlog_epsD, epsD)
