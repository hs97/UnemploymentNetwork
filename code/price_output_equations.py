...
Omega_c = np.array([0,0.2,0.8]) #this is our theta from before
Omega[0,:] = np.array([0.3,0.5])
Omega[1,:] = np.array([0.15,0.4])

Omega_L = np.array([0.1,0.3],dtype=np.float64)
lam     = np.array([0.6,0.5])

elasticities = np.ones((2,2))
shocks = np.array([0.5,-2])
...
def Lambda_mat(lam, Omega, Omega_c):
    I = np.eye(Omega.shape[0])
    lam_intermediate = np.tile(lam,(Omega.shape[1],1)) / (np.tile(lam,(Omega.shape[1],1)).T - np.tile(Omega_c,(Omega.shape[1],1)).T)
    Lambda = lam_intermediate * Omega.T 
    gamma = np.sum(Lambda,axis=1)
    Gam = np.linalg.inv(I - Lambda) @ Lambda - np.linalg.inv(I - Lambda) * np.tile(gamma,(Lambda.shape[1],1))
    return Lambda, gamma, Gam


def Xi_mat(Phi,Theta,Lambda,gamma,Gam):
    I = np.eye(Phi.shape[0])
    P1 = np.linalg.inv(I - Phi) * np.tile(Theta - 1,(Phi.shape[1],1))
    return P1 @ Gam, Gam

###### Pricing Equation ######
def prices_quantities(Omega, Omega_c, lam, elasticities, shocks):
    I = np.eye(Omega.shape[1])
    Theta = Theta_mat(Omega)
    Phi   = Phi_mat(Omega,Theta)
    Lambda, gamma, Gam = Lambda_mat(lam,Omega,Omega_c)
    Xi = Xi_mat(Phi,Theta,Lambda,gamma,Gam)  
...
###### code testing ######
dlog_p, dlog_x = prices_quantities(Omega, Omega_c, lam, elasticities, shocks)
