...
for i in range(lamvec.shape[0]):
    ...
    urateagg[i] = (eq.market1.Hi + eq.market2.Hi - eq.market1.Ld - eq.market2.Ld)/(eq.market1.Hi + eq.market2.Hi)
...
for i in range(lamvec.shape[0]):
    ...
    m2 = labor_market(lambdai=lamvec[i], lambdaj=lamvec[i], betai=b, Aj=shock)
    eq = economy(m1, m2)
    eq.solve_equilibrium()
    Lvec_s1[0, i], Lvec_s1[1, i] = eq.market1.Ld, eq.market2.Ld
    uratevec_s1[0, i], uratevec_s1[1, i] = eq.market1.urate, eq.market2.urate
    urateagg_s1 = (eq.market1.Hi + eq.market2.Hi - eq.market1.Ld - eq.market2.Ld)/(eq.market1.Hi + eq.market2.Hi)
...
ushock1    = (uratevec_s1 - uratevec)/uratevec*100
yshoc1     = (yvec_s1-yvec)/yvec
uaggshock1 = (urateagg_s1-urateagg)/urateagg*100
...
fig_unemployment_shock1, (plot1, plot2) = plt.subplots(2, 1)
plot1.plot(lamvec, ushock1[0, :],'-k')
plot1.set_ylabel('Sector 1')
plot1.invert_xaxis()
plot1.set_title("Percentage change in unemployment in response to a shock in sector 1")

plot2.plot(lamvec, ushock1[1, :],'-k')
plot2.set_ylabel('Sector 2')
plot2.set_xlabel('More labor flexibility ->')
plot2.invert_xaxis()
...
# responses of equilibrium outcomes to positive 1% shock to productivity in downstream sector at varying levels of labor market mobility
...
for i in range(lamvec.shape[0]):
    m1 = labor_market(lambdai=lamvec[i], lambdaj=1-lamvec[i], betai=0, Aj=shock)
    m2 = labor_market(lambdai=lamvec[i], lambdaj=1-lamvec[i], betai=b, Ai=shock)
    eq = economy(m1,m2)
    eq.solve_equilibrium()
    Lvec_s2[0,i], Lvec_s2[1,i] = eq.market1.Ld, eq.market2.Ld
    uratevec_s2[0,i], uratevec_s2[1,i] = eq.market1.urate, eq.market2.urate
    urateagg_s2 = (eq.market1.Hi + eq.market2.Hi - eq.market1.Ld - eq.market2.Ld)/(eq.market1.Hi + eq.market2.Hi)
...
ushock2 = (uratevec_s2 - uratevec)/uratevec*100
...
plot3.set_ylabel('Aggregate')
plot3.set_xlabel('More Labor Market Flexibility ->')
plot3.invert_xaxis()
...
for i in range(lamvec.shape[0]):
    ...
    urateagg_b = (eq.market1.Hi + eq.market2.Hi - eq.market1.Ld - eq.market2.Ld)/(eq.market1.Hi + eq.market2.Hi)
...
for i in range(lamvec.shape[0]):
    ...
    m2 = labor_market(lambdai=lam, lambdaj=lam, betai=bvec[i], Aj=shock)
    eq = economy(m1,m2)
    eq.solve_equilibrium()
    Lvec_bs[0,i], Lvec_bs[1,i] = eq.market1.Ld, eq.market2.Ld
    uratevec_bs[0,i], uratevec_bs[1,i] = eq.market1.urate, eq.market2.urate
    urateagg_bs[i] =  (eq.market1.Hi + eq.market2.Hi - eq.market1.Ld - eq.market2.Ld)/(eq.market1.Hi + eq.market2.Hi)
...
ushockb = (uratevec_bs-uratevec_b)/uratevec_b*100
...
fig_unemployment_shockb, (plot1, plot2) = plt.subplots(2, 1)
...
plot2.set_ylabel('Sector 2')
plot2.set_xlabel("More production linkages ->")


'''
plot3.plot(bvec,ushockb[1,:],'-k')
plot3.set_ylabel('Aggregate')
'''
